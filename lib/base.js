...
var Base = YeomanBase.extend( {
	...
	download: function( fileLocation, destination, options, cb ) {
		var fileName = path.basename( fileLocation ),
			installName = ( this.install ) ? this.install.name : '-bootstrap';
		options = _.assign( options || { type: 'http' }, this.options );

		if ( 'scp' === options.type ) {
			_.defaults(
				options,
				this.rcConfig.get( 'scp-' + installName + '-' + fileName ) || {},
				this.rcConfig.get( 'scp-' + fileName ) || {},
				this.rcConfig.get( 'scp-' + installName ) || {},
				this.rcConfig.get( 'scp' ) || {}
			);
		}

		return download( fileLocation, destination, options, cb );
	}
} );
