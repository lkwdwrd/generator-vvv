...
var Base = YeomanBase.extend( {
	...
	_isPathInVVV: function( workingPath, levels ) {
	...
	download: function( location, dest, opts, cb ) {
		var installName = ( this.install ) ? this.install.name : 'bootstrap';
		dest = ( path.isAbsolute( dest ) ) ? dest : this.destinationPath( dest );
		opts = _.assign( { type: 'http' }, opts || {}, this.options );
		opts.adapter = new Download.Adapter( _.assign(
			opts.adapterOpts || {},
			{
		...
		if ( ! opts.adapter.log.debug ) {
			opts.adapter.log.debug = debug;
		}
	...
	multiDownload: function( locations, opts, cb ) {
		var i, length, installName = ( this.install ) ? this.install.name : false;
		for ( i = 0, length = locations.length; i < length; i++ ) {
			if ( ! path.isAbsolute( locations[ i ].dest ) ) {
				locations[ i ].dest = this.destinationPath( locations[ i ].dest );
			}

			locations[ i ].opts = locations[ i ].opts || {};
			locations[ i ].opts.adapter = new Download.Adapter( _.assign(
				locations[ i ].opts.adapterOpts || {},
				{
					prompt: this.prompt.bind( this ),
					log: this.env.adapter.log
				}
			) );
			if ( ! locations[ i ].opts.adapter.log.debug ) {
				locations[ i ].opts.adapter.log.debug = debug;
			}

			if ( 'scp' === locations[ i ].opts.type ) {
				if ( installName ) {
					_.defaults(
						locations[ i ].opts,
						this.rcConfig.get( 'scp-' + installName ) || {}
